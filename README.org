...
With a compatible compiler and or_null, =Null= is treated as Jsonaf_kernel =`Null=, and =This= is
treated as the value contained, as shown below.
...
attribute =jsonaf.option= or =jsonaf.or_null= indicates that a record field should be optional.
/e.g./:
...
#+begin_src ocaml
  { x = Some 1; y = Some 2; z = This 3 } => {"x":1,"y":2,"z":3}
  { x = None  ; y = None; z = Null } => {"x":null}
#+end_src
...
#+begin_src ocaml
  type t =
    { a : u [@default u0] [@jsonaf_drop_default (=)]; (* explicit user-provided function *)
      ...
      f : u [@default u0] [@jsonaf_drop_default.compare.local]; (* uses [%compare.equal__local: u] *)
      g : u [@default u0] [@jsonaf_drop_default.equal.local]; (* uses [%equal__local: u] *)
    } [@@deriving jsonaf]
#+end_src
...
When using [[https://oxcaml.org][OxCaml]], you can derive a stack-allocating =of_json= function that accepts a
stack-allocated input. For example:
#+begin_src ocaml
type t = { a : string ; b : int } [@@deriving of_jsonaf ~stackify]
#+end_src

which generates
#+begin_src ocaml
val t_of_jsonaf : Jsonaf.t -> t
val t_of_jsonaf__stack : Jsonaf.t @ local -> t @ local
#+end_src

You can use this with =ppx_template= like
#+begin_src ocaml
let t = (t_of_jsonaf [@alloc stack]) json in
...
#+end_src
